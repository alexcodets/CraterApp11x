...
export const fetchDIDTOLLFREEs = ({
    ...
    return new Promise((resolve, reject) => {
        window.axios
            ...
            .catch((err) => {
                console.log("CATCH ERROR DE PROFILE DIDTOLLFREE:", err)
                reject(err)
            })
    })
}
...
export const addDIDTOLLFREE = ({
    ...
    return new Promise((resolve, reject) => {
        window.axios
            .post('/api/v1/profile/did-toll-free/insert', { prefijo: data.prefijo, status: data.statu.value, toll_free_category_id: data.toll_free_category_id, rate_per_minutes: data.rate_per_minutes })
            .then((response) => {
