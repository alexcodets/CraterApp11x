  ...
  <base-page class="customer-create">
      <sw-page-header class="mb-5" :title="pageTitle">
        ...
        <template slot="actions">
            ...
            :to="`/admin/customers/${$route.params.id}/payment-accounts/${$route.params.payment_account_id}/edit-${formData.payment_account_type}`"
            class="mr-3"
            variant="primary-outline"
            >
            {{ $t('general.edit') }}
            </sw-button>
            <sw-button slot="activator" variant="primary" @click="removePaymentAccount($route.params.payment_account_id)">
                {{ $t('general.delete') }}
            </sw-button>
        </template>
      </sw-page-header>
...
export default {
  ...
  computed: {
    ...
    isACH() {
      if (this.$route.name === 'customers.payment-account.view.ACH') {
        return true
      }
      return false
    },
    isCC() {
      if (this.$route.name === 'customers.payment-account.view.CC') {
        return true
      }
      return false
    },
  ...
  methods: {
    ...
    async loadPaymentAccount() {
      ...
      if(this.formData.card_number) {
        ...
        auxCardNumber.forEach((el, i) => {
          if(i < limit)
            ...
            showCardNumber = showCardNumber + el
        });
      ...
      if(this.formData.account_number) {
        const auxAccountNumber = this.formData.account_number.toString().split("");
        ...
        auxAccountNumber.forEach((el, i) => {
          if(i < limit)
            ...
            showAccountNumber = showAccountNumber + el
        });
        this.formData.account_number_pass = showAccountNumber;
      }
      if(this.formData.routing_number) {
        const auxRoutingNumber = this.formData.routing_number.toString().split("");
      ...
      swal({
        ...
        if (willDelete) {
          ...
          if (res.data.success) {
            ...
            this.$router.push(`/admin/customers/${this.$route.params.id}/payment-accounts`)
            return true
          } else {
