  ...
  <div :class="{ 'xl:pl-96': showSideBar}">
    <sw-page-header :title="$t('tickets.menu_title.departaments')">
      <sw-breadcrumb slot="breadcrumbs">
        <sw-breadcrumb-item to="#" :title="$tc('tickets.menu_title.departaments', 2)" active />
      </sw-breadcrumb>
      ...
      >
        ...
        >
          <template slot-scope="row">
            {{ row.default_priority == null ? '' : row.default_priority=='E' ? 'Emergency': row.default_priority=='C' ? 'Critical':  row.default_priority=='H' ? 'High' : row.default_priority=='M' ? 'Medium' : row.default_priority=='L' ? 'Low':'' }}
          </template>
        </sw-table-column>
...
export default {
  ...
  methods: {
    ...
    async fetchData({ page, filter, sort }) {
      ...
      console.log("respuesta de paquetes de pbx (LISTA):", response.data.departaments.data);
