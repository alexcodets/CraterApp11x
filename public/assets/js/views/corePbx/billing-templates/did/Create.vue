    ...
    <form action="" @submit.prevent="submitDID">
      ...
      <sw-page-header :title="pageTitle">
        <sw-breadcrumb slot="breadcrumbs">
          ...
          />
          <!-- <sw-breadcrumb-item
            v-if="$route.name === 'corepbx.did.edit'"
            :title="$t('corePbx.did.edit_did')"
            to="#"
            active
          />
          <sw-breadcrumb-item
            v-else-if="$route.name === 'corepbx.did.copy'"
            :title="$t('corePbx.did.copy_did')"
            to="#"
            active
          />
          <sw-breadcrumb-item
            v-else
            :title="$t('corePbx.did.new_did')"
            to="#"
            active
          /> -->
        </sw-breadcrumb>
                      ...
                      <div class="flex justify-start">
                        ...
                        >
                          <sw-input
                            ...
                            "
                            :placeholder="$t('corePbx.extensions.description')"
                            focus
                            type="text"
                            name="description"
                            tabindex="8"
...
export default {
  ...
  methods: {
    ...
    async loadDID() {
      ...
      this.formData.unmetered = res.data.profileDID.unmetered === 1 ? true : false
      ...
      if (this.formData.item_groups) {
                    this.formData.item_groups = res.data.profileDID.item_groups.map((itemGroup) => {
                        return {
                            ...itemGroup,
    ...
    async submitDID() {
      ...
      try {
        ...
        if (this.isEdit) {
          ...
          window.toastr['success'](this.$t('corePbx.did.did_update_message'))
          ...
          if (!this.isEdit) {
            window.toastr['success'](this.$t('corePbx.did.did_save_message'))
            this.$router.push('/admin/corePBX/billing-templates/did')
            return true
          }
        }
