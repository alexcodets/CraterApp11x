  ...
  <base-page v-if="isSuperAdmin" class="items">
    ...
    <div class="relative table-container" v-show="!showEmptyScreen">
      ...
      >
        ...
        <sw-table-column :sortable="true" :label="$t('corePbx.extensions.name')" show="name">
          <template slot-scope="row">
  ...
  methods: {
    ...
    async fetchData({ page, filter, sort }) {
      let data = {
        ...
        extensions_number: this.filters.extensions_number !== null ? this.filters.extensions_number : '',
      ...
      console.log('respuesta de EXTENSIONES (LISTA):', response)
      return {
      ...
      swal({
        ...
        try {
          if (willDelete) {
            ...
            window.toastr['success'](this.$t('corePbx.extensions.delete_extension'))
            this.$refs.table.refresh()
          }
        } catch (error) {
          window.toastr['error'](this.$t('corePbx.extensions.delete_extension_error'))
        } finally {
