        ...
        <form action="" @submit.prevent="submitCustomDidGroup">
          ...
          <table class="w-full text-center item-table mt-2">
            ...
            <th class="px-5 py-3 text-sm not-italic font-medium leading-5 text-right text-gray-700 border-t border-b border-gray-200 border-solid">
                <span class="pr-5">
                  {{ $t('corePbx.custom_did_groups.rate_per_minutes') }}
                </span>
            </th>
            </thead>
            <draggable
      ...
      try {
        ...
        if (this.isEdit) {
          ...
          if (response.data.success) {
            window.toastr['success'](this.$t('corePbx.custom_did_groups.updated_message'))
          ...
          if (response.data.success) {
            window.toastr['success'](this.$tc('corePbx.custom_did_groups.created_message'))
            this.$router.push('/admin/corePBX/billing-templates/custom-did-groups')
          }
          if (response.data.error) {
            this.isLoading = false
