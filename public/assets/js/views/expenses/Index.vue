      ...
      <sw-filter-wrapper v-show="showFilters" class="mt-3">
        ...
        >
          <sw-input
            ...
            :placeholder="$t(expenses.expense_number)"
            name="expense_number"
          />
        </sw-input-group>
      ...
      <sw-table-component ref="table" :show-filter="false" :data="fetchData">
        ...
        >
          <template slot-scope="row">
            ...
            <span> {{ row.provider_title ? row.provider_title : 'Not selected' }} </span>
          </template>
        </sw-table-column>
...
export default {
  ...
  created() {
    ...
    let objeto =  this.fetchProviders({ limit: 'all' })
   console.log(objeto);
  },
  ...
  methods: {
    ...
    async fetchData({ page, filter, sort }) {
      let data = {
        ...
        provider_id: this.filters.provider ? this.filters.provider.id : null,
        expense_category_id:
