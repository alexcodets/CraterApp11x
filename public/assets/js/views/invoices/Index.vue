        ...
        >
          <template slot-scope="row">
            ...
            <router-link
              ...
              :to="{ path: `invoices/${row.id}/view/${row.deleted_at}` }"
              class="font-medium text-primary-500"
            >
              {{ row.invoice_number }} 
            </router-link>
...
export default {
  ...
  methods: {
    ...
    setStatusFilter(val) {
      console.log('ACTIVE TAB & TITLE:', this.activeTab, val.title)
    ...
    async fetchData({ page, filter, sort }) {
      ...
      console.log('CARGO EL FETCHDATA INICIAL')
      return {
    ...
    setActiveTab(val) {
      console.log('SET ACTIVE TAB:', val)
      switch (val.value) {
        case 'DRAFT':
