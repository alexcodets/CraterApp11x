<template>
<div>
  <div v-if="!isView || selectedTax != null" class="flex items-center justify-between mb-3">
    <div class="flex items-center text-base" style="flex: 9; margin-left: 332px;">
      <label class="pr-2 mb-0" align="right">
        {{ $t('packages.tax') }}
      </label>
      <div v-if="isView">
        <p v-if="selectedTax != null && selectedTax.name != ''" class="text-sm font-bold leading-5 text-black non-italic" v-text="selectedTax.name"></p>
      </div>
    ...
    <div v-if="!isView" class="flex items-center justify-center w-6 h-10 mx-2 cursor-pointer">
      <trash-icon
...
export default {
  ...
  methods: {
    setTaxable(){
      this.disabled.no_taxable = !this.disabled.no_taxable
    },
