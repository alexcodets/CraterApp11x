...
export default {
  ...
  methods: {
    ...
    async loadPackage() {
      ...
      //console.log(this.$store.state.customer.packageParameters)
      ...
      if (this.isEdit) {
        ...
        this.package.items.forEach((item) => {
          ...
          if (item.itemTaxes.length > 0 && this.taxPerItem === 'YES') {
            obj.taxes = item.itemTaxes
          }

          obj.package_id = item.pivot.package_id
          obj.item_id = item.pivot.items_id
          obj.name = item.name
          obj.description = item.pivot.description
          obj.quantity = parseInt(item.pivot.quantity)
          obj.price = item.pivot.price
          obj.discount_type = item.pivot.discount_type
          obj.discount_val = item.pivot.discount_val
          obj.discount = this.discountPerItem === 'YES' ? item.pivot.discount : 0
          obj.total = item.pivot.total
    ...
    onSelectTax(selectedTax) {
      let amount = 0
      console.log('SELECCION DE TAX:', selectedTax)
    ...
    async next() {
      ...
      console.log(
        'DATOS QUE SE ALMACENAN EN PACKAGE PARAMETERS: - ADD SERVICES (DETAILS):',
        data
      )
