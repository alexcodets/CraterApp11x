        ...
        <form action="" @submit.prevent="submitAvalara">
          <sw-card>
            ...
            <sw-input-group
              ...
              class="md:col-start-1 md:col-span-3 mt-2"
            >
              <sw-select
            ...
            >
              <sw-select
                ...
                :show-labels="false"
                label="name"
                @select="companySelected($event, 'custom')"
              >
              </sw-select>
            </sw-input-group>
...
export default {
  ...
  computed: {
    ...
    cutomAppRateItemsOptions() {
      return this.avalaraItems.filter((item) => {
        if (item.avalara_bool && item.avalara_payment_type == 'LINES') {
          return item
        }
      })
    },
    itemsTypeTaxableAmountOptions() {
      return this.avalaraItems.filter((item) => {
        if (item.avalara_bool && item.avalara_payment_type == 'TAXABLE_AMOUNT') {
          return item
        }
      })
    },
  ...
  methods: {
    ...
    companySelected(val, type) {
      // console.log("🚀 ~ file: Create.vue ~ line 970 ~ companySelected ~ type", type)
      const vm = this
      switch (type) {
        case 'bscl':
