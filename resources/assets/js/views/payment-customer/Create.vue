...
.selected-option {
  ...
  /* Color de fondo al pasar el mouse o seleccionar */
}
...
.multiselect__option .multiselect__tag.status-SENT>span {
  ...
  /* Hereda el color de fondo de la etiqueta multiselect__tag */
}
    ...
    <form action="" @submit.prevent="submitPaymentData" id="payment-form">
      <sw-page-header :title="pageTitle" class="mb-5">
        <sw-breadcrumb slot="breadcrumbs">
          <sw-breadcrumb-item :title="$t('general.home')" to="/customer/dashboard" />
          <sw-breadcrumb-item :title="$tc('payments.payment', 2)" to="/customer/payments" />
          <sw-breadcrumb-item v-if="$route.name === 'payments.edit'" :title="$t('payments.edit_payment')" to="#"
            active />
          <sw-breadcrumb-item v-else :title="$t('payments.new_payment')" to="#" active />
        </sw-breadcrumb>
      ...
      <div v-if="!isShowIdentificationVerification">
        <sw-card  v-if="!isRequestOnGoing">
          <div class="grid-cols-12 gap-8 mt-1 mb-2 lg:grid">
            <!-- First Section -->
            <div class="grid grid-cols-1 col-span-12 gap-4 mt-8 lg:gap-4 lg:mt-0 lg:grid-cols-3">
              <!-- date -->
              <sw-input-group :label="$t('payments.date')" :error="DateError" required>
                <base-date-picker v-model="formData.payment_date" :invalid="$v.formData.payment_date.$error"
                  :calendar-button="true" class="mt-1" calendar-button-icon="calendar" :disabled="true"
              ...
              <sw-input-group :label="$t('payments.invoice_title')">
                <div class="button-group">
                  <sw-button type="button" class="button-all" :class="{ 'button-active': activeButton === 'ALL' }"
                    @click="filtrarYSeleccionarInvoices('ALL')">{{ $t('general.all') }}</sw-button>
                  <sw-button type="button" class="button-sent" :class="{ 'button-active': activeButton === 'SENT' }"
                    @click="filtrarYSeleccionarInvoices('SENT')">{{ $t('general.sent') }}</sw-button>
                  <sw-button type="button" class="button-due" :class="{ 'button-active': activeButton === 'VIEWED' }"
                    @click="filtrarYSeleccionarInvoices('VIEWED')">{{ $t('general.due') }}</sw-button>
                  <sw-button type="button" class="button-overdue"
                    :class="{ 'button-active': activeButton === 'OVERDUE' }"
                    @click="filtrarYSeleccionarInvoices('OVERDUE')">{{ $t('general.overdue') }}</sw-button>
                </div>
                <sw-select v-model="selectedInvoices" :options="invoiceList" :multiple="true" :searchable="true"
                  :show-labels="false" :allow-empty="true" :placeholder="$t('invoices.select_invoice')"
                  :custom-label="invoiceWithAmount" class="custom-sw-select mt-1" track-by="invoice_number"
                  label="invoice_number" :search-keys="['invoice_number']">
                  <template #option="{ option, selected, select }">
                    <div
                      ...
                      <input type="checkbox" :value="option" v-model="selectedInvoices" @change="select(option)"
              ...
              <!-- amount -->
              <sw-input-group :label="$t('payments.amount')" :error="amountError" required>
                <div class="relative w-full mt-1">
                  <sw-money v-model="amount" :currency="customerCurrency" :invalid="$v.formData.amount.$error"
                    :disabled="isEdit" class="relative w-full focus:border focus:border-solid focus:border-primary-500"
              ...
              <sw-input-group v-if="creditv">
                <div class="">
                  <span class="flex flex-wrap justify-start">{{ $t('payments.account_avalable_credit') }}:
                    <div class="text-success text-xl ml-2" v-html="$utils.formatMoney(
      ...
      " />
                  ...
                  <sw-button class="flex flex-wrap justify-end" :loading="isLoading" :disabled="isLoading"
                    variant="primary" type="button" @click="paymentWithCustomerBalance">
            ...
            <!-- Second Section -->
            <div v-if="customerHasPaymentMethods"
              ...
              <div>
                <sw-input-group :label="$t('payments.payment_mode')">
                  <sw-select v-model="formData.payment_method" :options="options_payment_methods" :searchable="true"
                    :show-labels="false" :placeholder="$t('payments.select_payment_mode')" :max-height="150"
                    label="name" class="mt-1" :error="paymentMethodError" @input="PaymentModeSelected" required>
                  </sw-select>
                </sw-input-group>
              </div>

              <!-- Paypal -->
              <div class="mt-5" v-if="formData.payment_method &&
      formData.payment_method.paypal_button &&
      customer != null
      ">
                <paypal :formData="formData" :codePayment="codePayment" :invoice_number="invoice_number"
                  :customer="customer" @paypalSuccess="paypalSuccess"></paypal>
              </div>

              <!-- CC and ACH-->
              <div class="mt-1 ml-12">
                <!-- type_cc and billing -->
                <div v-if="this.add_payment_gateway_select &&
      !this.isEdit &&
      !this.type_ach &&
      this.type_cc &&
      ...
      ">
                  <div style="
                    ...
                    align-items: center;
                  ">
                    <img :src="paymentAccountPreview.src" :width="paymentAccountPreview.width" />
                  ...
                  ">
                    <p style="padding-left: 0; padding-right: 0px">
                      <b style="color: #3939ff">{{ $t('customers.billing_address') }}:</b>
      ...
      !this.type_cc &&
      !isLoadingThePaymentAccount &&
      paymentAccountPreview != null && !isIdentificationVerification
      ">
              ...
              <div class="mx-0 mt-0 flex justify-end">
                <sw-dropdown position="bottom-end">
                  <sw-button v-if="!isIdentificationVerification" :disabled="isLoading" :loading="isLoading"
                    slot="activator" class="ml-4" variant="primary" type="button">
                  ...
                  <sw-dropdown-item v-if="isPaymentAccounts" @click="changeAccountModal">
            ...
            <div v-else
              ...
              <div class="mt-12 mb-6">
                <sw-badge class="no_document" :bg-color="$utils.getBadgeStatusColor('I').bgColor"
...
<!--           <div v-if="customerHasPaymentMethods && !isShowIdentificationVerification"
          ...
          <div v-if="!isIdentificationVerification"
            class="pt-2 pb-4 grid grid-cols-1 col-span-12 gap-4 mt-8 lg:gap-4 lg:mt-0 lg:grid-cols-1">
            <div  class="w-full flex justify-end">
              <sw-button v-if="!notEditable" :loading="isLoading" :disabled="isLoading || !customerHasPaymentMethods"
                variant="primary" type="submit" size="lg" class="sm:flex">
                <save-icon v-if="!isLoading" class="mr-2 -ml-1" />
                {{
      isEdit ? $t('payments.update_payment') : $t('invoices.pay_now')
    }}
              </sw-button>
          ...
          <sw-button v-if="isIdentificationVerification" :loading="isLoading" :disabled="isLoading" variant="primary"
            type="submit" size="lg" class="flex md:mt-4">
          ...
          <div v-if="this.paymentfeesenabled && this.payment_fees.length > 0
      " class="w-full md:pr-2">
            <div>
              ...
              <div class="grid grid-cols-1 md:grid-cols-4 gap-4 text-sm">
                <div v-for="(item, index) in payment_fees" :key="index" class="p-2 border rounded">
...
export default {
  ...
  created() {
    if ( this.settingsCompany.enable_make_customer === "0") {
    ...
    window.hub.$on('changePaymentAccount', (val) => {
      this.setPaymentAccount(val, true)
    })
  },
  async mounted() {
  ...
  methods: {
    ...
    async selectFirstInvoices() {
      ...
      if (this.$route.params.id && !isNaN(this.$route.params.id)) {
        ...
        // Resto del código...
        if (invoiceObject !== null && invoiceObject !== undefined && typeof invoiceObject === 'object') {
          // Verifica si el campo id está en this.invoiceList y this.selectedInvoices
          const isInInvoiceList = this.invoiceList.some(invoice => invoice.id === invoiceObject.id);
          const isInSelectedInvoices = this.selectedInvoices.some(invoice => invoice.id === invoiceObject.id);
          ...
          console.log('invoiceObject no es un objeto válido o es null/undefined');
        }
      } else {
    ...
    async setPaymentAccountFromPaymentMethodSelector(payment_method_type) {
      if (this.payment_accounts.length > 0) {
        ...
        if (payment_account != undefined) {
          this.setPaymentAccount(payment_account, false, 'selector')
        } else {
    ...
    async setPaymentAccount(payment_account, fromModal, from = 'default') {
    ...
    async PaymentModeSelected(val) {
      ...
      if (val.add_payment_gateway === 1) {
        let res = await this.fetchPaymentGateways()
        console.log("RES PAYMENT MODE GATEWAYS", )
        if (res) {
          this.payment_gateways = res.data.payment_gateways
          this.payment_gateways.forEach((element) => {
