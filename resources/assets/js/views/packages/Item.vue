...
.tax-row {
  ...
  margin-bottom: 10px; /* Ajusta este valor para el espacio entre filas */
}

.tax-name,
.tax-percent,
.tax-amount {
  margin-left: 20px; /* Ajusta este valor para el espacio entre columnas */
}
</style>
        ...
        <tbody>
          <tr style="background-color: #f5f3f3">
            ...
            <td class="px-5 py-4 text-right align-top">
              <div v-if="isView" class="flex items-center justify-end text-sm">
                <span>
                  <div v-html="item.quantity" />
                </span>
              </div>
              <sw-input
                v-else
          ...
          <tr v-if="taxPerItem === 'YES'" class="tax-tr">
            <!-- <td class="px-5 py-4 text-left align-top" /> -->
            <td colspan="12" class="px-5 py-4 align-top">
              <div>
                <!-- Cuando isView es true, recorre e imprime el array item.taxes -->
                <div v-if="isView">
                  <p style="float: left; margin-bottom: 5px">
                    {{ $t('packages.packages_tax_item') }}
                  </p>
                  <div
                  ...
                  >
                    ...
                    <div class="tax-percent">{{ tax.percent }}%</div>
                    <div
                      class="tax-amount"
                      v-html="$utils.formatMoney(tax.amount, currency)"
                    ></div>
                  </div>
                </div>

                <!-- Cuando isView es false, muestra el componente tax con las propiedades vinculadas -->
                <tax
                  v-else
                  v-for="(tax, index) in item.taxes"
                  :key="tax.id"
                  :disabled="item.no_taxable == 1"
                  :index="index"
                  :tax-data="tax"
                  :taxes="item.taxes"
...
export default {
  ...
  mounted() {
    this.$v.item.$reset()
    // console.log('Mounted isView', this.isView)
  },
  methods: {
    updateTax(data) {
    ...
    onSelectItem(item) {
      ...
      this.item.no_taxable = item.no_taxable
    ...
    updateItem() {
      // console.log('items: ', this.item.taxes);
      this.$emit('update', {
        index: this.index,
        item: {
          ...this.item,
          ...
          tax: this.totalTax,
          taxes: this.item.taxes ? [...this.item.taxes] : [],
        },
      })
    },
    removeItem() {
