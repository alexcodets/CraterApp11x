    ...
    <form action="" @submit.prevent="submitMetho">
      ...
      <sw-page-header class="mb-3" :title="pageTitle">
        <sw-breadcrumb slot="breadcrumbs">
          ...
          />
          <sw-breadcrumb-item
            to="/customer/reports/departaments"
            :title="$t('tickets.menu_title.departaments')"
          />
          <sw-breadcrumb-item
            v-if="$route.name === 'corepbx.customer.DepartamentsPbxEdit'"
            to="#"
...
export default {
  ...
  computed: {
    ...
    pageTitle() {
      if (this.$route.name === 'corepbx.customer.DepartamentsPbxEdit') {
    ...
    async submitMetho() {
      ...
      try {
        if (this.isEdit) {
          ...
          this.$router.push({ name: 'corepbx.customer.departaments' })
        } else {
          swal({
