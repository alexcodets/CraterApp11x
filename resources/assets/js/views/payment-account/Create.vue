...
export default {
  ...
  methods: {
    ...
    async loadPaymentAccount() {
     ...
     // console.log(response.data.payment_accounts)
      if (response.data.payment_accounts.state_id) {
       // console.log(response.data.payment_accounts)
        //console.log(response.data.payment_accounts.state_id)
        //console.log(response.data.payment_accounts.state)
    ...
    async fetchInitData() {
      ...
      if (response.data.user && this.isEditACH == false && this.isEditCC==false) {
      ...
      if (permissions.super_admin == false) {
        if (permissions.exist == false) {
          ...
          if (modulePermissions.create == 0 && this.isACH == true) {
            this.$router.push('/admin/dashboard')
          } else if (modulePermissions.create == 0 && this.isCC == true) {
            this.$router.push('/admin/dashboard')
          } else if (modulePermissions.update == 0 && this.isEditACH == true) {
            this.$router.push('/admin/dashboard')
          } else if (modulePermissions.update == 0 && this.isEditCC == true) {
            this.$router.push('/admin/dashboard')
          }
        }
      }
    ...
    async copyAddress(val) {
      if (val === true) {
        ...
        if (res) {
          this.billing_state = res.data.states.find(
            ({ id }) => id == response.data.customer.billing_address.state_id
          )
