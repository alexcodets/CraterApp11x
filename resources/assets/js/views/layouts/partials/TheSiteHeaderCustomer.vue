<template>
  <header class="
    ...
    <a href="/admin/dashboard" class="
        ...
        font-base
      ">
      <img v-if="previewLogo" id="logo-white" :src="previewLogo" class="hidden h-6 md:block" />
      <!--     <img
        v-else
        id="logo-white"
      ...
      />-->
      <img v-if="previewLogo" id="logo-mobile" :src="previewLogo" class="block h-8 md:hidden" />
      <!--      <img
        v-else
        id="logo-mobile"
        src="/assets/img/logo-corebill.png"
    ...
    <ul class="float-right h-8 m-0 list-none md:h-9">
      <a :class="{ 'is-active': isSidebarOpen }" href="#" class="
          flex
          float-left
          p-1
          ml-3
      ...
      <li class="relative block float-left ml-2">
        <sw-dropdown>
          <a slot="activator" href="#" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"
            class="inline-block text-sm text-black bg-white rounded-sm avatar">
            <img :src="profilePicture" alt="Avatar" class="w-8 h-8 rounded-sm md:h-9 md:w-9" />
          </a>

          <sw-dropdown-item v-if="showAccount" tag-name="router-link" :to="'/customer/payment-accounts'">
            <credit-card-icon class="w-4 h-4 mr-2 text-gray-600" />
            {{ $t('navigation.payment_accounts') }}
          </sw-dropdown-item>

          <sw-dropdown-item v-if="showTickets" tag-name="router-link" :to="'/customer/tickets'">
...
export default {
  ...
  methods: {
    ...
    updateMenuVisibility() {
      ...
      this.showAccount = this.settingsCompany.enable_paymentaccount_customer === "1";
