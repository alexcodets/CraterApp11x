<template>
  <div class="customer-create">
    <sw-page-header :title="$t('settings.mobile.notifications_logs.title')">
      <sw-breadcrumb slot="breadcrumbs">
        <sw-breadcrumb-item :title="$t('general.home')" to="dashboard" />
        <sw-breadcrumb-item
          :title="$tc('settings.mobile.notifications_logs.title', 2)"
          to="#"
          active
        />
      </sw-breadcrumb>
    ...
    <sw-empty-table-placeholder
      ...
      :title="$t('settings.mobile.notifications_logs.no_logins')"
      :description="$t('settings.mobile.notifications_logs.list_of_logins_desc')"
    >
    ...
    <div v-show="!showEmptyScreen" class="relative table-container">
      ...
      >
        <p class="text-sm">
          {{ $t('general.showing') }}: <b>{{ toLogs }}</b>
          {{ $t('general.of') }} <b>{{ totalLogs }}</b>
        </p>
      ...
      >
        <sw-table-column
          :sortable="true"
          :filterable="true"
          :label="$tc('settings.mobile.notifications_logs.customer', 1)"
          show="name"
        >
          <template slot-scope="row">
            <span>{{ $tc('settings.mobile.notifications_logs.customer', 1) }}</span>
              <!-- v-if="row.customer" -->
            <span
              @click="customerDetail(row.customer.id)"
              class="font-medium text-primary-500 cursor-pointer"
            >
              {{ row.customer.customcode }} - {{ row.customer.name }}
            </span>
          </template>
        </sw-table-column>
        ...
        <sw-table-column
          :sortable="true"
          :label="$t('settings.mobile.notifications_logs.date')"
          show="formattedCreatedAt"
        >
          <template slot-scope="row">
            <span>{{ $t('settings.mobile.notifications_logs.date') }}</span>
            <span>
        ...
        <sw-table-column
          :sortable="true"
          :label="$t('settings.mobile.notifications_logs.message')"
          show="message"
        >
          <template slot-scope="row">
            <span>{{ $t('settings.mobile.notifications_logs.message') }}</span>
            <span>
              {{ row.message }}
            </span>
          </template>
        ...
        <sw-table-column
          :sortable="true"
          :label="$t('settings.mobile.notifications_logs.notification_sent')"
          show="notification_sent"
        >
          <template slot-scope="row">
            <span>{{ $t('settings.mobile.notifications_logs.notification_sent') }}</span>
            <span>
              {{ row.notification_sent === 1 ? 'Sended' : 'Not sended' }}
            </span>
          </template>
        </sw-table-column>
      </sw-table-component>
...
export default {
  ...
  computed: {
    showEmptyScreen() {
      return !this.totalLogs && !this.isRequestOngoing
    },
    filterIcon() {
  ...
  methods: {
    ...
    async fetchData({ page, filter, sort }) {
      ...
      let response = await this.fetchNotificationsLogs(data)
      this.totalLogs = response.data.notificationsLogs.total
      this.toLogs = response.data.notificationsLogs.data.length
      this.isRequestOngoing = false

      return {
        data: response.data.notificationsLogs.data,
        pagination: {
          totalPages: response.data.notificationsLogs.last_page,
          currentPage: page,
        },
      }
    },
