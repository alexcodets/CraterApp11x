...
export default {
  ...
  computed: {
    showEmptyScreen() {
      return !this.totalauthorizationall
    },
    filterIcon() {
  ...
  methods: {
    ...
    this.totalauthorizationall= response.data.authorizationTotalCount
      return {
    ...
    async setDefault(auth){
      let res = await this.setAuthorizeDefault(auth);
      if (res.data.success){
