...
export default {
  ...
  methods: {
    ...
    ...mapActions('lead', ['fetchLeads','sendEmaillead']),
    ...
    async setInitialData() {
      ...
      if (this.modalData) {
        ...
        this.formData.to = this.modalData.email
      }
      ...
      this.formData.body = res.data.customer_lead_notification_body
      if (res.data.customer_lead_notification_subject) {
        if (res.data.customer_lead_notification_subject != null) {
          this.formData.subject = res.data.customer_lead_notification_subject
        }
      }
    },
    resetFormData() {
        ...
        try {
          if (value) {
            ...
            if(!result.status){
              ...
              console.log(data)
              this.isLoading = true
              let res = await this.sendEmaillead(data)
              this.closeModal()
              if (res.data.success) {
                this.isLoading = false
                window.toastr['success'](
                  this.$tc('general.lead_email_success')
                )
