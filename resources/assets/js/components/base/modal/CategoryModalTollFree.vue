<template>
  <form action="" @submit.prevent="submitCategoryData">
    <div class="p-8 sm:p-6">
      ...
      <sw-input-group
        :label="$t('corePbx.didFree.description')"
...
export default {
  ...
  methods: {
    ...
    ...mapActions('categoriesTollF', ['addCategory', 'updateCategory']),
    resetFormData() {
      this.formData = {
        id: null,
        name: null,
      ...
      if (response.data) {
        if (!this.isEdit) {
          window.toastr['success'](
            this.$t('corePbx.didFree.created_message_category')
          )
        } else {
          window.toastr['success'](
            this.$t('corePbx.didFree.updated_message_category')
          )
        }
