...
export default {
  ...
  getBadgeStatusExpenseColor(status) {
    switch (status) {
      case 'Active':
        return {
          bgColor: '#D5EED0',
  ...
  /**
 * Cancels the current form and displays a confirmation message to the user.
 *
 * @param {Object} lang - Internationalization (i18n) object that provides the $t function.
 * @param {Object} router - Vue Router object for navigation.
 * @param {String} type -  String to define the type of the Swal button, whether it represents a cancel or a back action. string 'cancel' or string 'back'
 */
  cancelFormOrBack(lang, router, type) {

    if(type == "cancel"){
       swal({
      title: lang.$t('general.are_you_sure'),
      text: lang.$t('general.cancel_text'),
      icon: 'error',
      buttons: true,
      dangerMode: true,
    }).then(async (result) => {
