...
namespace Crater\Services\Payment\Authorize\Util;

use net\authorize\util\MapperObj;
...
class Mapper
{
    ...
    public function getClass($class, $property): ?MapperObj
    {
        ...
        if (isset($this->classes[$class]['properties'][$property]['type'])) {
            ...
            if (stripos($className, "DateTime<'Y-m-d'>") !== false) {
                $className = 'Date';
            } elseif (stripos($className, "DateTime") !== false) {
                $className = 'DateTime';
            }
